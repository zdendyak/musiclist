...
export const albumRandomFailure = error => ({ type: 'MUSIC_ALBUM_RANDOM_FAILURE', error });
export const albumRandomSuccess = json => ({ type: 'MUSIC_ALBUM_RANDOM_SUCCESS', json });
...
export function addAlbum(id) {
  return async (dispatch) => {
    ...
    // Send packet to our API, whitch will communicate with Discogs
    await fetch(
      // where to contact
      '/api/albums/add',
      // what to send
...
// Get the latest album from Discogs API
    ...
    await fetch(
      ...
      .then((json) => {
        if (json.results) {
          ...
          return dispatch(albumRandomSuccess(json.results[index]));
        }
        return dispatch(albumRandomFailure(new Error(json.error)));
      })
      .catch(error => dispatch(albumRandomFailure(new Error(error))));
