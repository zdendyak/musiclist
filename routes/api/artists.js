...
router.post('/add', async (req, res) => {
  // Make sure a user's is actually logged in
  if (!req.user) {
  ...
  try {
    ...
    if (!artistSaved) { throw new Error('There was a problem saving the artist to the database'); }
    ...
    result = res.json({ error: 'There was a problem saving the artist to the database. Please try again.' });
  }
  return result;
});
...
router.post('/delete', (req, res, next) => {
  User.findOne({ username: req.user.username }, (err, foundUser) => {
    // Run filter against the array, returning only those that don't match passed ID
