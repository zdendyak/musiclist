...
// configure Discogs
const discogsClient = new Discogs('BandsList-oz/0.1', {
...
const saveArtists = async (artists) => {
  const formattedAtrists = artists.map((artist) => {
...
router.post('/add', async (req, res) => {
  ...
  try {
    ...
    if (!albumSaved) { throw new Error('There was a problem saving the album to the database. Album could not be saved.'); }
    ...
    if (!artistsSaved) { throw new Error('There was a problem saving the album to the database. Artists could not be saved'); }
    ...
    result = res.json({ error: err.message || 'There was an error saving the album to the database. Please try again.' });
  }
  return result;
});
...
router.post('/delete', (req, res, next) => {
  User.findOne({ username: req.user.username }, (err, foundUser) => {
    // Run filter against array, returning only those that don't match passed ID
